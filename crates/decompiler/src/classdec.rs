...
fn emit_class(
    ...
    // Source-level overload collisions (same name + same parameter
    // types, different return — only era/generated bytecode has these):
    // suffix the 2nd+ occurrences so the unit compiles.
    {
        ...
        for mi in 0..pc.cf.methods.len() {
            ...
            let Some(name) = pc.method_name(mi) else { continue };
            ...
            *n += 1;
            if *n > 1 {
                renames.insert(mi, format!("{}$jc{}", name, *n - 1));
            }
        }
        METHOD_RENAME.with(|r| *r.borrow_mut() = renames);
    }
    let mut method_emit_result = Ok(());
    for mi in 0..pc.cf.methods.len() {
        if skip.contains(&mi) {
            continue;
        }
    ...
    METHOD_RENAME.with(|r| r.borrow_mut().clear());
    method_emit_result?;
...
fn emit_method_with(
    ...
    let m = &pc.cf.methods[mi];
    let name = pc.utf8(m.name_index).unwrap_or("?").to_string();
    // Hidden-method collision rename (era bytecode): see METHOD_RENAME.
    let name = METHOD_RENAME
        .with(|r| r.borrow().get(&mi).cloned())
        .unwrap_or(name);

//! Local variable tables, JVM side: derive `jdc_core::var::VarTable` from the
//! method's `LocalVariableTable` / `LocalVariableTypeTable`.
//!
//! The table *type* is machine-neutral and lives in the core (a DEX front-end
//! synthesizes it from register live ranges instead); this module only knows
//! how a class file records names and generic types.
...
pub use jdc_core::var::{VarInfo, VarTable};

/// Register a variable with an explicit live range (the core's public
/// constructors cover the synthetic cases; the LVT builder needs ranges).
fn add_var(
    vt: &mut VarTable,
    slot: u16,
    name: String,
    ...
    vt.vars.push(VarInfo { id, slot, name, ty, is_param, range_start: rs, range_end: re, synthetic_name: synth });
    while vt.by_slot.len() <= slot as usize {
        vt.by_slot.push(Vec::new());
    }
    vt.by_slot[slot as usize].push((rs, re, id));
    id
}
...
/// Build the variable table for a method from its LocalVariableTable /
/// LocalVariableTypeTable.
pub fn build_var_table(
    pc: &PoolClass,
    m_idx: usize,
    ...
    if !is_static {
        add_var(&mut vt, slot, "this".into(), JavaType::Object(pc.internal_name.clone()).into(), true, 0, code_len, false);
    ...
    for (i, a) in desc.args.iter().enumerate() {
        ...
        param_slots.push(slot);
        add_var(&mut vt, slot, name, ty, true, 0, code_len, false);
    ...
    for (start, end, name, d, s) in &lvt {
        ...
        add_var(&mut vt, *s, name.clone(), ty, false, *start, (*end).max(*start + 1), false);
    }
    ...
    for s in 0..slots_needed {
        ...
        if !covered {
            ...
            add_var(&mut vt,s, name, TypeRef::J(JavaType::Int), !is_static && s == 0, 0, code_len, true);
        }
    }
...
fn sig_matches_base(tr: &TypeRef, base: &JavaType) -> bool {
    ...
    match g {
        ...
        GenericType::TypeVar(_) => matches!(base, JavaType::Object(_)),
        GenericType::Class(cs) => {
            matches!(base, JavaType::Object(n) if *n == cs.internal_name())
        }
        GenericType::Primitive(c) => base.primitive_char() == Some(*c),
        GenericType::Array(_) => matches!(base, JavaType::Array(_)),
        GenericType::Wildcard(_) => false,
    }
}
